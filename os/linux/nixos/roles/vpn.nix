  ...
  wvpn = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      EOF
          ...
          # 【未解決 2026-08-25】この二段フローの前提は誤りだった。FortiClient は同一 IKE_SA を
          # 開いたまま2巡目の EAP でトークンを送っており、1回目を失敗させる方式ではない。
          # さらに FortiOS 7.6.7 は EAP-Request/GTC の Code バイトを 1 でなく 3 (Success) で
          # 送るため、RFC 準拠クライアントはトークン要求に応答できない (詳細は docs/roles/vpn.md)。
          # ゲートウェイ側が修正されるまで本フローは機能しない。正しい形は修正後に再設計する。
