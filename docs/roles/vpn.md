...
- **OTP メールはパスワード付きの認証試行が届いた時点で発送される** (FortiClient で
  「接続」を押した後にメールが来るのと同じ)。手元にコードが無い状態で `wvpn up` した
  場合は OTP プロンプトを**空 Enter** すると、パスワードのみの試行を先に打って
  メール送信をトリガーする (この試行は失敗するのが期待動作)。届いた 6 桁を続けて
  入力すれば本試行になる。
